...
class SpeechAssistantApp:
    ...
    def create_widgets(self):
        ...
        top_bar.pack(fill=tk.X, padx=12, pady=(8, 2))

        tk.Label(top_bar, text="Speech Optimizer Pro", font=("Segoe UI", 18, "bold"), fg="#1a237e").pack(side=tk.LEFT)
        tk.Label(top_bar, text="  Advanced Scientific Emotion Analysis", font=("Segoe UI", 11), fg="#555").pack(side=tk.LEFT, pady=4)

        status_frame = tk.Frame(top_bar)
        status_frame.pack(side=tk.RIGHT, anchor="e")
        tk.Label(status_frame, text=self.loader.base_status, font=("Consolas", 8), fg="#2e7d32").pack(anchor="e")
        tk.Label(status_frame, text=self.loader.emobank_status, font=("Consolas", 8), fg="#1565c0").pack(anchor="e")
        tk.Label(status_frame, text=self.loader.benchmark_status, font=("Consolas", 8), fg="#7b1fa2").pack(anchor="e")
        audio_status = "[OK] Live audio enabled" if AUDIO_RECORD_AVAILABLE else "[WARN] sounddevice not installed"
        tk.Label(status_frame, text=audio_status, font=("Consolas", 8), fg="#e65100").pack(anchor="e")
        asr_status = "[OK] ASR enabled" if ASR_AVAILABLE else "[INFO] ASR (SpeechRecognition) not installed"
        tk.Label(status_frame, text=asr_status, font=("Consolas", 8), fg="#00838f").pack(anchor="e")

        notebook = ttk.Notebook(self.root)
        notebook.pack(fill=tk.BOTH, expand=True, padx=12, pady=5)

        self.single_frame = ttk.Frame(notebook)
        notebook.add(self.single_frame, text="  Text Analysis  ")
        self.create_single_tab()

        self.live_frame = ttk.Frame(notebook)
        notebook.add(self.live_frame, text="  Live Recording  ")
        self.create_live_tab()

        self.audio_file_frame = ttk.Frame(notebook)
        notebook.add(self.audio_file_frame, text="  Audio File Analysis  ")
        self.create_audio_file_tab()

        self.bench_frame = ttk.Frame(notebook)
        notebook.add(self.bench_frame, text="  Benchmark Comparison  ")
    ...
    def create_single_tab(self):
        ...
        left_frame = ttk.LabelFrame(content_frame, text="Speech Text Input")
        left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=4)

        scene_frame = ttk.Frame(left_frame)
        scene_frame.pack(fill=tk.X, padx=6, pady=6)
        ttk.Label(scene_frame, text="Speech Context:").pack(side=tk.LEFT)
        self.scene_var = tk.StringVar(value="Academic Competition")
        scene_combo = ttk.Combobox(
            scene_frame, textvariable=self.scene_var,
            values=["Academic Competition", "Corporate Presentation", "Commemorative Address"],
            state="readonly", width=22
        )
        scene_combo.pack(side=tk.LEFT, padx=5)
        scene_combo.bind("<<ComboboxSelected>>", self.change_scene)

        self.text_input = scrolledtext.ScrolledText(left_frame, wrap=tk.WORD, font=("Consolas", 10), undo=True)
        self.text_input.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)

        btn_frame = ttk.Frame(left_frame)
        btn_frame.pack(fill=tk.X, padx=6, pady=6)
        ttk.Button(btn_frame, text="Analyze", command=self.analyze_single).pack(side=tk.LEFT, padx=3)
        ttk.Button(btn_frame, text="Clear", command=lambda: self.text_input.delete("1.0", tk.END)).pack(side=tk.LEFT, padx=3)
        ttk.Button(btn_frame, text="Export Report", command=self.export_single_report).pack(side=tk.RIGHT, padx=3)
        ttk.Button(btn_frame, text="Reload Corpora", command=self.reload_all).pack(side=tk.RIGHT, padx=3)

        right_frame = ttk.LabelFrame(content_frame, text="Analysis Report")
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=4)

        self.result_text = scrolledtext.ScrolledText(right_frame, wrap=tk.WORD, font=("Consolas", 9), state=tk.DISABLED)
    ...
    def create_live_tab(self):
        if not AUDIO_RECORD_AVAILABLE or not MATPLOTLIB_AVAILABLE:
            ...
            tk.Label(self.live_frame, text=tip, font=("Segoe UI", 12), fg="#e65100", justify="center").pack(pady=100)
            return

        control_bar = ttk.Frame(self.live_frame)
        control_bar.pack(fill=tk.X, padx=12, pady=10)

        self.record_btn = ttk.Button(control_bar, text="Start Recording", command=self.toggle_recording)
        self.record_btn.pack(side=tk.LEFT, padx=5)

        self.live_status_var = tk.StringVar(value="Ready. Click Start Recording to begin.")
        ttk.Label(control_bar, textvariable=self.live_status_var, foreground="#666").pack(side=tk.LEFT, padx=15)

        ttk.Button(control_bar, text="Generate Report", command=self.generate_live_report).pack(side=tk.RIGHT, padx=5)

        plot_frame = ttk.LabelFrame(self.live_frame, text="Real-time Loudness Waveform & Emotion Intensity Curve")
        plot_frame.pack(fill=tk.BOTH, expand=True, padx=12, pady=5)

        self.live_fig = Figure(figsize=(10, 3.5), dpi=100)
        ...
        self.live_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True, padx=6, pady=6)

        self.ax_loudness.set_title("Vocal Intensity (RMS, normalized)", fontsize=10)
        self.ax_loudness.set_ylim(0, 1)
        self.ax_loudness.set_ylabel("Loudness")
        self.line_loudness, = self.ax_loudness.plot([], [], color="#e53935", linewidth=1.5)

        self.ax_emotion.set_title("Affective Intensity (confidence-weighted)", fontsize=10)
        self.ax_emotion.set_ylim(0, 1)
        self.ax_emotion.set_ylabel("Intensity")
        self.ax_emotion.set_xlabel("Time (seconds)")
        self.line_emotion, = self.ax_emotion.plot([], [], color="#1976d2", linewidth=1.5)

        info_frame = ttk.LabelFrame(self.live_frame, text="Current State")
        ...
        tk.Label(info_frame, textvariable=self.current_emo_var, font=("Segoe UI", 11, "bold"), fg="#d32f2f").pack(side=tk.LEFT, padx=15, pady=8)
        tk.Label(info_frame, textvariable=self.current_loud_var, font=("Segoe UI", 10)).pack(side=tk.LEFT, padx=15, pady=8)
        tk.Label(info_frame, textvariable=self.current_pitch_var, font=("Segoe UI", 10)).pack(side=tk.LEFT, padx=15, pady=8)
        tk.Label(info_frame, textvariable=self.duration_var, font=("Segoe UI", 10)).pack(side=tk.RIGHT, padx=20, pady=8)

        result_frame = ttk.LabelFrame(self.live_frame, text="Recording Analysis Report")
        result_frame.pack(fill=tk.BOTH, expand=True, padx=12, pady=5)

        self.live_result_text = scrolledtext.ScrolledText(result_frame, wrap=tk.WORD, font=("Consolas", 9), state=tk.DISABLED, height=8)
        self.live_result_text.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)

    def create_audio_file_tab(self):
        """New tab: import audio file for acoustic analysis + optional ASR transcription."""
        top_frame = ttk.Frame(self.audio_file_frame)
        top_frame.pack(fill=tk.X, padx=12, pady=12)

        ttk.Label(top_frame, text="Audio File (.wav):", font=("Segoe UI", 10)).pack(side=tk.LEFT)
        self.audio_path_var = tk.StringVar()
        ttk.Entry(top_frame, textvariable=self.audio_path_var, width=55).pack(side=tk.LEFT, padx=6)
        ttk.Button(top_frame, text="Browse", command=self.select_audio_file).pack(side=tk.LEFT, padx=4)
        ttk.Button(top_frame, text="Analyze Audio", command=self.analyze_audio_file).pack(side=tk.LEFT, padx=10)

        if ASR_AVAILABLE:
            ttk.Button(top_frame, text="Transcribe to Text (ASR)", command=self.transcribe_audio).pack(side=tk.LEFT, padx=4)
            tk.Label(top_frame, text="(Google API, requires internet)", font=("Segoe UI", 8), fg="#888").pack(side=tk.LEFT, padx=2)

        content_frame = ttk.Frame(self.audio_file_frame)
        content_frame.pack(fill=tk.BOTH, expand=True, padx=12, pady=5)

        # Plot area
        if MATPLOTLIB_AVAILABLE:
            plot_frame = ttk.LabelFrame(content_frame, text="Audio Emotion Curve")
            plot_frame.pack(fill=tk.X, padx=2, pady=2)
            self.audio_fig = Figure(figsize=(10, 2.5), dpi=100)
            self.ax_audio = self.audio_fig.add_subplot(111)
            self.audio_fig.tight_layout()
            self.audio_canvas = FigureCanvasTkAgg(self.audio_fig, master=plot_frame)
            self.audio_canvas.get_tk_widget().pack(fill=tk.X, padx=6, pady=6)
            self.ax_audio.set_title("Loudness & Emotion Over Time", fontsize=10)
            self.ax_audio.set_xlabel("Time (seconds)")
            self.line_audio_loud, = self.ax_audio.plot([], [], color="#e53935", linewidth=1, label="Loudness")
            self.line_audio_pitch, = self.ax_audio.plot([], [], color="#1976d2", linewidth=1, label="Pitch Variation")
            self.ax_audio.legend(loc="upper right", fontsize=8)

        # Report area
        report_frame = ttk.LabelFrame(content_frame, text="Audio Analysis Report")
        report_frame.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
        self.audio_result_text = scrolledtext.ScrolledText(report_frame, wrap=tk.WORD, font=("Consolas", 9), state=tk.DISABLED)
        self.audio_result_text.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)
    ...
    def create_benchmark_tab(self):
        if not self.comparer.available():
            tk.Label(
                ...
                font=("Segoe UI", 12), fg="#999", justify="center"
            ).pack(pady=80)
            return

        left_frame = ttk.LabelFrame(self.bench_frame, text="Benchmark Speech Library")
        left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=8, pady=8)

        self.bench_listbox = tk.Listbox(left_frame, width=45, height=28)
        self.bench_listbox.pack(padx=6, pady=6)
        ...
        ttk.Button(left_frame, text="Compare", command=self.do_compare).pack(pady=5)

        right_frame = ttk.LabelFrame(self.bench_frame, text="Comparison Results")
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=8, pady=8)

        self.compare_text = scrolledtext.ScrolledText(right_frame, wrap=tk.WORD, font=("Consolas", 10), state=tk.DISABLED)
        self.compare_text.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)

    def create_batch_tab(self):
        top_frame = ttk.Frame(self.batch_frame)
        top_frame.pack(fill=tk.X, padx=12, pady=12)

        ttk.Label(top_frame, text="Speech Folder:", font=("Segoe UI", 10)).pack(side=tk.LEFT)
        self.folder_var = tk.StringVar()
        ttk.Entry(top_frame, textvariable=self.folder_var, width=55).pack(side=tk.LEFT, padx=6)
        ttk.Button(top_frame, text="Browse", command=self.select_folder).pack(side=tk.LEFT, padx=4)
        ttk.Button(top_frame, text="Run Batch Analysis", command=self.run_batch).pack(side=tk.LEFT, padx=10)

        table_frame = ttk.LabelFrame(self.batch_frame, text="Batch Results Overview")
        table_frame.pack(fill=tk.BOTH, expand=True, padx=12, pady=5)

        columns = ("Filename", "Score", "Grade", "Words", "Duration", "Opening", "Ending", "Quotable", "Readability", "Findings", "Primary Finding")
        self.tree = ttk.Treeview(table_frame, columns=columns, show="headings", height=20)

        col_widths = [170, 55, 75, 55, 65, 75, 75, 65, 75, 65, 220]
        ...
        bottom_frame = ttk.Frame(self.batch_frame)
        bottom_frame.pack(fill=tk.X, padx=12, pady=10)
        ttk.Label(bottom_frame, text="CSV export contains 19 detailed metrics.", foreground="#666").pack(side=tk.LEFT)
        ttk.Button(bottom_frame, text="Export CSV Report", command=self.export_batch).pack(side=tk.RIGHT)
